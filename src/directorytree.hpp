...
/// \class TreeEntry
/// Describes a directory entry in a DirectoryTree - either a file or a directory.
class TreeEntry {
public:
...
public:
  ...
  [[nodiscard]] virtual Subscription subscribe_to_changes(const std::function<void(const DirectoryTreeDiff&)>& callback) const = 0;
