...
class ConsoleInterruptHandler {
 ...
 public:
  ...
  /// interrupts the process. An interrupt that arrived before this method is
  /// called (but after the `ConsoleInterruptHandler` is constructed) is not
  /// missed. `std::stop_token`s returned from this function will not be
  /// triggered after the creating `ConsoleInterruptHandler` is destroyed.
  [[nodiscard]] std::stop_token token() const;
};

/// @class ConsoleInterruptHandlerTestUtil
/// Provides a test-only injection point for `ConsoleInterruptHandler`'s failure
/// handling. It exists so that the constructor's failure path, which otherwise
/// depends on the operating system refusing to install the handler, can be
/// exercised deterministically. It is not intended for production use.
class ConsoleInterruptHandlerTestUtil {
 public:
